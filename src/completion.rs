...
//
//     // Get document content with a short-lived lock
//     let content = match self.document_map.lock().await.get(&document_uri) {
//       Some(content) => content.clone(),
//       None => return Ok(None),
//     };
//
//     // Extract word prefix at current position
//     if let Some(prefix) = self.get_word_prefix(&content, position) {
//       // Debug output - only if prefix exists
//       if !prefix.is_empty() {
//         eprint!("prefixes: {}\n", prefix);
//
//         // Find matching words and return them
